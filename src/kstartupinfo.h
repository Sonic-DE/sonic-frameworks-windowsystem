...
public:
    ...
    /*!
     * Creates and returns new startup id with \a timestamp as user timestamp part.
     *
     ...
     * \value CleanOnCantDetect when a new unknown window appears, all startup
     *     notifications for applications that are not compliant with
     *     the startup protocol are removed
     * \value DisableKWinModule KWinModule, which is normally used to detect
     *     new windows, is disabled. With this flag, checkStartup() must be
     *     called in order to check newly mapped windows.
     * \value AnnounceSilenceChanges normally, startup notifications are
     ...
     *
     * \a parent the parent of this QObject (can be \c nullptr for no parent)
     *
     */
    ...
    /*!
     * Like sendStartup , uses \a conn instead of QX11Info::connection() for sending the info.
    ...
    /*!
     * Like sendChange , uses \a conn instead of QX11Info::connection() for sending the info.
    ...
    /*!
     * Like sendFinish , uses \a conn instead of QX11Info::connection() for sending the info.
     * \a conn the xcb connection of the application.
     * \a screen The x11 screen the connection belongs to
     * \a id the id of the application
     * Returns true if successful, false otherwise
     ...
     * \value NoMatch    the window doesn't match any existing startup notification
     * \value Match      the window matches an existing startup notification
     * \value CantDetect unable to detect if the window matches any existing
     *          startup notification
     */
    enum startup_t { NoMatch, Match, CantDetect };
    /*!
     * Checks if the given windows matches any existing startup notification.
     * \a w the window id to check
     ...
     * \a w the window id to check
     * \a id if found, the id of the startup notification will be written here
     * Returns the result of the operation
     */
     ...
     * \a w the window id to check
     * \a data if found, the data of the startup notification will be written here
     * Returns the result of the operation
     */
...
public:
     ...
     * \a bin the new binary name of the application
     */
     ...
     * Returns the name of the startup notification, or an empty string
     *         if not set.
     */
     ...
     * Returns the name of the startup notification's icon, or the name of
     ...
     * \a desktop the desktop for the startup notification
     */
    ...
    /*!
     * Checks whether the given \a pid is in the list of PIDs for startup
     * notification.
     * Returns true if the given \a pid is in the list of PIDs for the startup notification
     */
    bool is_pid(pid_t pid) const;
    /*!
     * Sets the hostname on which the application is starting. It's necessary to set
     * it if PIDs are set.
     * \a hostname the application's hostname. If it's a null string, the current hostname is used
     */
     ...
     * should be silenced (temporarily suspended) with the given \a state.
     */
    void setSilent(TriState state);

    /*!
     * Return the silence status for the startup notification.
     * Returns KStartupInfoData::Yes if visual feedback is silenced
     */
     ...
     * the starting application should appear ).
     * \a xinerama the Xinerama screen for the startup notification
     */
