...
class BackgroundEffectManager : public QWaylandClientExtensionTemplate<BlurManager>, public QtWayland::ext_background_effect_manager_v1
{
    Q_OBJECT
public:
...
void WindowEffects::installBlur(QWindow *window, bool enable, const QRegion &region)
{
    ...
    if (m_backgroundEffectManager->isActive()) {
        ...
        if (!effect) {
            qWarning() << "effect mgr obj" << ext_background_effect_manager_v1_get_background_effect(m_backgroundEffectManager->object(), surface);
