...
void WindowSystem::exportWindow(QWindow *window)
{
    ...
    // We want to use QObject::property(char*) and use danymic properties on the object rather than
...
void WindowSystem::setMainWindow(QWindow *window, const QVariant &handle)
{
    if (!window) {
        return;
    }
    ...
    const QString parentHandle = handle.toString();
    ...
    if (window->isExposed()) {
        doSetMainWindow(window, parentHandle);
    } else {
        ...
        window->setProperty(c_kdeXdgForeignPendingHandleProperty, QVariant(parentHandle));
...
void WindowSystem::doSetMainWindow(QWindow *window, const QString &parentHandle)
{
    Q_ASSERT(window);
    Q_ASSERT(!parentHandle.isEmpty());
    ...
    WaylandXdgForeignImportedV2 *imported = importer.importToplevel(parentHandle);
