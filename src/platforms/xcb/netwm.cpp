...
void NETWinInfo::event(xcb_generic_event_t *event, NET::Properties *properties, NET::Properties2 *properties2)
{
    ...
    Properties effectiveDirtyProperties = dirty & p->properties;
    Properties2 effectiveDirtyProperties2 = dirty2 & p->properties2;

    // We *always* want to update WM_STATE if set in dirty_props
    if (dirty & XAWMState) {
        effectiveDirtyProperties |= XAWMState;
    }

    if (do_update) {
        update(effectiveDirtyProperties, effectiveDirtyProperties2);
    }

    if (properties) {
        *properties = effectiveDirtyProperties;
    }
    if (properties2) {
        *properties2 = effectiveDirtyProperties2;
    }
}
